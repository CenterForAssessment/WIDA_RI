#########################################################################################
###                                                                                   ###
###   WIDA Rhode Island Learning Loss Analyses -- 2020 Baseline Growth Percentiles    ###
###                                                                                   ###
#########################################################################################
...
if (length(tmp.names <- grep("BASELINE|SS", names(WIDA_RI_SGP_LONG_Data))) > 0) {
		WIDA_RI_SGP_LONG_Data[,eval(tmp.names):=NULL]
}
...
data.table::setnames(WIDA_RI_SGP_LONG_Data,
	c("SCALE_SCORE_PRIOR", "SCALE_SCORE_PRIOR_STANDARDIZED"),
	c("SS_PRIOR_COHORT", "SS_PRIOR_STD_COHORT"))

SGPstateData[["WIDA_RI"]][["Assessment_Program_Information"]][["CSEM"]] <- NULL

WIDA_RI_SGP <- abcSGP(
        sgp_object = WIDA_RI_SGP_LONG_Data,
...
data.table::setnames(WIDA_RI_SGP@Data,
